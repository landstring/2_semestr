...
int task(tree* tr, tree* v, int h) {
	if (!v->left && !v->right) {
		return h;
	}
	else {
		if (v->left && v->right) {
			int r = task(tr, v->right, h + 1);
			int l = task(tr, v->left, h + 1);
			if (r > l) {
				return r;
			}
			else {
			...
			return task(tr, v->left, h + 1);
		}
		else {
			return task(tr, v->right, h + 1);
		}
	}
}
