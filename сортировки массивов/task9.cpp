...
int** gapSort_start(int** &mas, int n) {
	for (int col = 0; col < n; col++) {
		if (col % 3 == 2) {
			int* a = new int[n];
			for (int i = 0; i < n; i++) { //âçÿëè ñòîëáåö
				a[i] = mas[i][col];
			}

			a = gapSort(a, n); //îòñîðòèðîâàëè ñòîëáåö 

			for (int i = 0; i < n; i++) { //çàïèñàëè îòñîðòèðîâàííûé ñòîëáåö âìåñòî èñõîäíîãî â ìàññèâ 
				mas[i][col] = a[i];
			}
			delete[] a; //óäàëèëè ïðîìåæóòî÷íûé ñòîëáåö èç ïàìÿòè
		}
	}
	return mas;
